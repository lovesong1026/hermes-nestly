...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    try:
        # Keep this zero-argument call intentionally: it is the canonical
        # primary-provider path and also preserves integrations that replace
        # the resolver with a compatible callable.
        runtime = resolve_runtime_provider()
    except AuthError as auth_exc:
        if new_api_url:
            from gateway.web.upstream_key import normalize_new_api_base_url
            logger.info(
                "BYO-key mode: no global provider key configured; "
                "web_chat will inject the end-user key per request"
            )
            return {
...
def _try_resolve_fallback_provider() -> dict | None:
    """Use Hermes' shared fallback walker after primary authentication fails.

    The primary attempt above deliberately stays a plain resolver call so
    BYO-key web deployments can distinguish a missing server key.  This helper
    delegates the actual fallback policy to upstream rather than maintaining a
    fork-specific config parser.
    """
    from hermes_cli.auth import AuthError
    ...
    if fallback_entry is None:
        return None
    model_cfg = fallback_entry.get("model") or ""
    return {
...
_BUILTIN_ADAPTERS: dict[Platform, tuple[str, str, str, str]] = {
    ...
    Platform.WEB_CHAT: ("web_chat", "WebChatAdapter", "check_web_chat_requirements",
                        "Web Chat: aiohttp/argon2-cffi not installed"),

...
_TERMINAL_CWD_OVERRIDE: ContextVar[str | object] = ContextVar("_TERMINAL_CWD_OVERRIDE", default=_UNSET)
...
def set_terminal_cwd_override(path: str | Path | None) -> Token:
    """Set a task-local working-directory hint for isolated integrations.

    This is intentionally a ContextVar rather than ``TERMINAL_CWD`` in the
    process environment: concurrent web requests must never change each
    other's working directory hint.
    """
    value: str | object = _UNSET if path is None else str(path)
...
def reset_terminal_cwd_override(token: Token) -> None:
    """Restore the previous terminal-CWD override."""
    _TERMINAL_CWD_OVERRIDE.reset(token)


def get_terminal_cwd() -> str | None:
    """Return the active task-local terminal-CWD hint, if any."""
